...
with open("config.json") as f:
    ...
    config["channels"]["search"] = [int(env_vars["SEARCH_CHANNEL"])]
    config["channels"]["misc"] = [int(env_vars["MISC_CHANNEL"])]
    config["channels"]["mod"] = [int(env_vars["MOD_CHANNEL"])]
    config["channels"]["match"] = [int(env_vars["MATCH_CHANNEL"])]
...
channelList = [
    ...
    context.miscChannels,
    context.modChannels,
    context.matchChannels,
]
# flatten the list
...
attributes = {
    "WIFI": {"symbol": wifiSymbol, "color": wifiColor},
    "NETP": {"symbol": netpSymbol, "color": netpColor},
}
...
with open("prerankingsave.json") as f:
    ...
    fixedprerankingsN = {
        int(key): value for key, value in loadprerankings["NETP"].items()
    }
    fixedprerankingsW = {
        int(key): value for key, value in loadprerankings["WIFI"].items()
    }
...
def findPlayer(LBType, standing):
    ...
    sortedRankingList = dict(
        sorted(strRankingList.items(), key=lambda x: x[1], reverse=True)
    )
...
def buildLBoard(rankingList, listType, page, matchType):
    ...
    for index in range(part):
        ...
        player_block = f"{standing}) {playerName}".ljust(
            41
        )  # max name length is 32 + room for the #) part
...
async def characters(ctx):
    ...
    if channelOrigin in channelList:
        await ctx.send(
            "I've sent you a DM with all character inputs I can currently recognize."
        )
...
async def changerank(ctx):
    ...
    if (
        winnerID not in rankings[ladderType].keys()
        or loserID not in rankings[ladderType].keys()
    ):
    ...
    calculateELO(
        winnerID, loserID, ladderType, endCheck=False, modCheck=True, wNeeded=2
    )
...
async def top(ctx):
    ...
    if ctx.message.channel.id not in context.miscChannels:
        await ctx.message.delete()
        await ctx.send(
            f"This command can only be used in {channels_to_str(context.miscChannels)}",
            delete_after=10,
        )
        return
    ...
    if topType == "WIFI" or topType == "NETP":
        ...
        await ctx.send(
            "Please specify what type of leaderboard you'd like to view. (NETP or WIFI)",
            delete_after=10,
        )
...
async def rank(ctx):
    ...
    if (
        ctx.message.content[6:10].upper() == "NETP"
        or ctx.message.content[6:10].upper() == "WIFI"
    ):
        boardType = ctx.message.content[6:10].upper()
        standing = ctx.message.content[11:]
        if standing == "":
            await ctx.send(
                "Please specify what player standing you'd like to search for \n (ie. ``-rank wifi 3``)",
                delete_after=8,
            )
            return
    ...
    embed = discord.Embed(title=f"{playerInfo}", colour=discord.Colour(defaultColor))
    ...
    if playerID in rankings["NETP"]:
        ...
        embed.add_field(
            name=f"{netpSymbol} NETP ELO: {showNetpELO}",
    ...
    if playerID in rankings["WIFI"]:
        ...
        embed.add_field(
            name=f"{wifiSymbol} WIFI ELO: {showWifiELO}",
...
async def ranked(ctx):
    ...
    if ctx.message.channel.id not in context.searchChannels:
        await ctx.message.delete()
        await ctx.send(
            f"This command can only be used in {channels_to_str(context.searchChannels)}",
            delete_after=10,
        )
        return
    if banCheck(ctx.message.author.id):
...
async def on_reaction_add(reaction, user):
    ...
    if (
        reaction.message.author.id != botID
    ):  # Checks if reaction to message is not Brawlbot's
        return

    if (
        messageID in searchMessages.keys() or messageID in matches.keys()
    ):  # Checks if this message is one that expects reactions on
        ...
        if (
            reaction.emoji == abort and challengerID == playerID
        ):  # only the person that queues up can cancel the search
        ...
        # event when someone clicks on challenge
            ...
            if user.id in players2matches.keys():
                ...
                await channel.send(
                    f"<@{user.id}>You are currently in a match.", delete_after=4
                )
                return
            setType = searchMessages[messageID]["setType"]
            matchType = searchMessages[messageID]["matchType"]
            ...
            challengeMessages[dmChallenge.id] = {
        ...
        # event when players are in a match and want to cancel
            ...
            reaction.emoji == cancel and user.id in matches[messageID]["players"].keys()
        ):
            ...
            if len(matches[messageID]["cancel"].keys()) == 2:
                ...
                rankings[matchType][user.id] = matches[messageID]["players"][user.id][
                    "elo"
                ]
                rankings[matchType][opponent] = matches[messageID]["players"][opponent][
                    "elo"
                ]
                saveRankings(rankings)
        ...
        # event when players click on a reaction of a stage
        elif (str(reaction.emoji) in matches[messageID]["stages"]) or (
            str(reaction.emoji) == noBan
        ):
                ...
                if user.id == matches[messageID]["banning"]:
                    ...
                    if len(matches[messageID]["stages"]) == 5:
                        ...
                        newEmbed.set_thumbnail(
                            url=context.stageTNs[matches[messageID]["stages"][0]]
                        )
                        await reaction.clear()
                        await matchWindowObj.clear_reaction(
                            matches[messageID]["stages"][0].strip("<:>")
                        )
                        await matchWindowObj.edit(embed=newEmbed)
                        for icon in winloss:
            ...
            # game 2 and onwards procedures
                ...
                if user.id == matches[messageID]["banning"]:
                    ...
                    charaSelect = discord.Embed(
                        title=f"[Game {gameCount} | {stage} {context.emotes2stage[stage]}]",
        ...
        # event when win or loss is clicked
            ...
            str(reaction.emoji) in winloss
            and user.id in matches[reaction.message.id]["players"]
        ):
            messageID = reaction.message.id
            ...
            if (
                str(reaction.emoji) == win
                and opponents[user.id] != matches[messageID]["loser"]
            ):
                return

            elif (
                str(reaction.emoji) == loss
                and opponents[user.id] != matches[messageID]["winner"]
            ):
                return
            ...
            if dsl:
                ...
                matches[messageID]["stages"] = remainingstagelist(
                    fullstagelist[:], matches[messageID]["players"][loser]["dsl"]
                )
            ...
            # if set is over
            if (
                matches[messageID]["players"][user.id]["wins"]
                == matches[messageID]["winsNeeded"]
                or matches[messageID]["players"][opponents[user.id]]["wins"]
                == matches[messageID]["winsNeeded"]
            ):
                ...
                rmWindow = 30
                embed.set_footer(
                    text=f"Both players can click {rm} to initiate a rematch"
                )
            ...
            # otherwise
                ...
                embed = matchWindowObj.embeds[0]
                embed.add_field(
                    name=f"Game {matches[messageID]['gameCount']}",
                    value=f""
                    f"\n"
                ...
                matches[rematchWindow.id] = {
                    "players": {
                        ...
                        challengerID: {
                            "character": "N/A",
                            "wins": 0,
                            "elo": rankings[matchType][challengerID],
                            "dsl": [],
                        },
                    },
        ...
        # smashdown related reactions
            ...
            sdplayers[user.id] = {
                "sdgameID": messageID,
                "messageObj": sdgames[messageID]["messageObj"],
                "opponent": opponent,
            }
            sdplayers[opponent]["opponent"] = user.id
            sdgames[messageID]["players"][user.id] = {
                "character": "n/a",
                "wins": 0,
                "selecting": False,
                "usedCharacters": [],
            ...
            newEmbed = discord.Embed(
                title="Smashdown!",
                description=showroster(
                    sdgames[messageID]["remaining"], context.characterlist
                ),
                color=defaultColor,
            )
        ...
        # if player is trying to leave a game
            ...
            if len(sdgames[reaction.message.id]["playerlist"]) == 1:
                ...
                rosterResults.description = (
                    f"{showused(client.get_user(user.id), sdgames[sdgameID]['players'][user.id]['usedCharacters'])}\n"
                ...
                updating.set_field_at(
                    index=userfield,
                    name=f"{client.get_user(user.id)}",
                    value=f"Wins: {sdgames[sdgameID]['players'][user.id]['wins']} 🏆\n\n",
                )

                updating2.set_field_at(
                    index=oppfield,
                    name=f"{client.get_user(opponent)}",
        ...
        # if win or loss is reported on smashdown
            ...
            if nextgame and sdgames[sdgameID]["remaining"] != 0:
                ...
                # if the roster is down to 1 character
                    ...
                    sdgames[reaction.message.id]["players"][loser][
                        "character"
                    ] = finalchara
                    sdgames[reaction.message.id]["players"][winner][
                        "character"
                    ] = finalchara

                    sdgames[sdgameID]["players"][winner]["usedCharacters"].append(
                        finalchara
                    )
                    sdgames[sdgameID]["players"][loser]["usedCharacters"].append(
                        finalchara
                    )

                    updating.set_field_at(
                        index=loserfield,
                        name=f"{client.get_user(loser)} {finalchara}",
                        value=f"Wins: {sdgames[sdgameID]['players'][loser]['wins']} 🏆\n\n",
                    )

                    updating2.set_field_at(
                        index=winnerfield,
                        name=f"{client.get_user(winner)} {finalchara}",
                        value=f"Wins: {sdgames[sdgameID]['players'][winner]['wins']} 🏆\n\n",
                    )

                    updatedRoster = sdgameObj.embeds[0]
                    updatedRoster.description = showroster(
                        sdgames[sdgameID]["remaining"], context.characterlist
                    )
                ...
                # if there are no more characters
                    ...
                    if (
                        sdgames[sdgameID]["players"][winner]["wins"]
                        > sdgames[sdgameID]["players"][loser]["wins"]
                    ):
                        winnermark = "👑"
                    ...
                    updating.set_field_at(
                        index=loserfield,
                        name=f"{losermark} {client.get_user(loser)} {losermark}",
                        value=f"Wins: {sdgames[sdgameID]['players'][loser]['wins']} 🏆\n\n",
                    )

                    updating2.set_field_at(
                        index=winnerfield,
                        name=f"{winnermark} {client.get_user(winner)} {winnermark}",
                        value=f"Wins: {sdgames[sdgameID]['players'][winner]['wins']} 🏆\n\n",
                    )

                    rosterResults = sdgameObj.embeds[0]
                    rosterResults.description = (
                        f"{showused(client.get_user(winner), sdgames[sdgameID]['players'][winner]['usedCharacters'])}\n"
...
async def on_raw_reaction_add(payload):
    ...
    if (
        payload.message_id not in challengeMessages.keys()
        and playerID not in players2matches.keys()
    ):
        # print("not in there")
        return
    ...
    # event for when accept is clicked on
        ...
        selectedChara = discord.Embed(
            title=f"{matches[matchWindowID]['players'][playerID]['character']} selected!",
        ...
        if matches[matchWindowID]["heading"] == "N/A":
            ...
            charaSelect = discord.Embed(
                title=f"[Game {gameCount} | {stage} {stageName}]",
...
async def on_message(message):
    ...
    if message.guild in client.guilds and authorID in sdplayers.keys():
        ...
        if sdgames[sdgameID]["players"][authorID]["selecting"]:
            if content in context.brawlChara.keys():
                ...
                sdgames[sdgameID]["players"][authorID]["usedCharacters"].append(
                    charaIcon
                )
                ...
                selectEmbed.set_field_at(
                    index=fieldnum,
                    name=f"{client.get_user(authorID)} {charaIcon}",
                ...
                if sdgames[sdgameID]["players"][opponentID]["character"] == "n/a":
                    ...
                    opponentEmbed.set_field_at(
                        index=opponentField,
                        name=f"{client.get_user(opponentID)}",
    ...
    if authorID in players2matches.keys():
        ...
        if matches[matchWindowID]["gameCount"] == 1:
            ...
            matches[matchWindowID]["players"][authorID]["character"] = (
                context.brawlChara[character]
            )
            selectedDM = await message.author.create_dm()
            selectedChara = discord.Embed(
                title=f"{matches[matchWindowID]['players'][authorID]['character']} selected!",
                description=f"Return to matchmaking: Click here -> {matchchannel}",
                color=defaultColor,
            )
            selectedChara.set_thumbnail(
        ...
        # event for character selections game 2 and onwards
            ...
            if matches[matchWindowID]["heading"] == "N/A":
                ...
                playerCharacter = matches[matchWindowID]["players"][authorID][
                    "character"
                ]
                gameCount = matches[matchWindowID]["gameCount"]
                selectDM = await opponentDM.create_dm()
